...
+!check_bored
	...
	<- .random(X); .wait(X*5000+2000); // From time to time, I get bored...
		.send(robot, askOne, time(_), Response); // ...so I ask robot about the time (notice request is SYNCHRONOUS)
		.print(Response); 
		!!check_bored. // notice "parallel" intention execution (è come se ci fosse un new thread)
